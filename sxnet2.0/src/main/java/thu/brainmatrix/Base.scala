...
import thu.brainmatrix.util.NativeLibraryLoader
...
object Base {
  ...
  //by lxg
  type ScalaSymbolHandle = SymbolHandle
  type ScalaSymHandle = SymbolHandle
  type ScalaSymbolHandleRef = SymbolHandleRef 
  type OperatorPropertyHandle = CPtrAddress
  type OperatorPropertyHandleRef = RefLong
  type ScalaSymHandleRef =RefLong
  type StaticGraphHandle =CPtrAddress
  type StaticGraphHandleRef =RefLong
  var INPUTSHAPE_AUXILIARY = Map[String,Shape]()
  var TYPE_NN = "lstm"

  try {
    try {
      tryLoadLibraryOS("mxnet")
    } catch {
    ...
    case e: UnsatisfiedLinkError =>
      logger.error("Couldn't find native library brainmatrix-scala")
      throw e
  }
