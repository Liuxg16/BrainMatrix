...
object Executor {
  ...
  private[brainmatrix] def getDict(names: Seq[String],
   ...
   */
  private[brainmatrix] def splitInputSlice(batchSize: Int,
   ...
   */
  private[brainmatrix] def checkArguments(symbol: Symbol): Unit = {
  ...
  private[brainmatrix] def loadGeneral(data: Seq[NDArray], targets: Seq[NDArray]): Unit = {
  ...
  private[brainmatrix] def loadGeneralMulti(data: Seq[NDArray],
...
//        println(s"dst:${dst.shape}")
//        println(s"sliced:${sliced.shape}\t")
        sliced.copyTo(dst)
        sliced.dispose()
      }
    }
  ...
  private[brainmatrix] def loadDataMulti(batch: DataBatch,
                                   targets: Seq[Array[(Int, Int, NDArray)]]): Unit = {
    loadGeneralMulti(batch.data, targets)
  }

  private[brainmatrix] def loadData(batch: DataBatch, targets: Seq[NDArray]): Unit = {
    loadGeneral(batch.data, targets)
  }

  // Load label into sliced arrays
  private[brainmatrix] def loadLabelMulti(batch: DataBatch,
                                    targets: Seq[Array[(Int, Int, NDArray)]]): Unit = {
    loadGeneralMulti(batch.label, targets)
  }

  private[brainmatrix] def loadLabel(batch: DataBatch, targets: Seq[NDArray]): Unit = {
...
class Executor private[brainmatrix](private[brainmatrix] val handle: ExecutorHandle,
                              private[brainmatrix] val symbol: Symbol) {
  private[brainmatrix] var argArrays: Array[NDArray] = null
  private[brainmatrix] var gradArrays: Array[NDArray] = null
  private[brainmatrix] var auxArrays: Array[NDArray] = null
  ...
  private[brainmatrix] var _ctx: Context = null
  private[brainmatrix] var _gradsReq: Iterable[_] = null
  private[brainmatrix] var _group2ctx: Map[String, Context] = null
...
class DataParallelExecutorManager(symbol: Symbol,
  ...
  private[brainmatrix] val _paramNames = paramIdx.map(argNames(_))
  private[brainmatrix] val paramArrays = paramIdx.map { i =>
    trainExecs.map(_.argArrays(i))
  }.toArray
  private[brainmatrix] val gradArrays = paramIdx.map { i =>
  ...
  private[brainmatrix] val cpuOutputArrays = outputShapes.map(NDArray.zeros(_))
