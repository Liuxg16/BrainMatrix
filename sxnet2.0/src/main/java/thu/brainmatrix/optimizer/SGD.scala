package thu.brainmatrix.optimizer

import thu.brainmatrix.{Optimizer, LRScheduler, NDArray}
import thu.brainmatrix.NDArrayConversions._
...
class SGD(private val learningRate: Float = 0.01f, private val momentum: Float = 0.0f,
  ...
  override def update(index: Int, weight: NDArray, grad: NDArray, state: AnyRef): Unit = {
    ...
    if (state != null) {
      val mom = state.asInstanceOf[NDArray]//NDArray.zeros(weight.shape, weight.context)
      mom *= momentum
