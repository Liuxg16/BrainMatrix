 ...
 * \file concatlxg.h
 * \brief support for concatlxgenation
 */
#ifndef MSHADOW_EXTENSION_CONCATLXG_H_
#define MSHADOW_EXTENSION_CONCATLXG_H_
...
namespace expr {
/*!
 * \brief concatlxg expression, concatlxg two tensor's channel
...
struct concatlxgExp : public TRValue<concatlxgExp<LhsExp, RhsExp,
                                            Device, DType,
                                            srcdim, dimsrc_m_cat>,
                                  Device, srcdim, DType> {
  ...
  index_t dcat_src2_;
  Shape<4> shape_;
  concatlxgExp(const LhsExp &src1, const RhsExp &src2) : src1_(src1), src2_(src2) {
    Shape<srcdim> sshape1 = ShapeCheck<srcdim, LhsExp>::Check(src1_);
    Shape<srcdim> sshape2 = ShapeCheck<srcdim, RhsExp>::Check(src2_);
    #pragma unroll
    for (int i = 0; i < srcdim; ++i) {
      if (i != dimcat) {
        CHECK_EQ(sshape1[i], sshape2[i]) << "concatlxgExp: shape mismatch";
      }
    }
    this->shape_ = sshape1;
...
/*!
 * \brief concatlxg two 4D tensor
 * \param src1 source tensor1
 * \param src2 source tensor2
 * \return concatlxged 4D tensor
 * \tparam cdim the dimension to concatlxgnate on
...
inline concatlxgExp<LhsExp, RhsExp, Device, DType, srcdim, srcdim - cdim>
concatlxg(const TRValue<LhsExp, Device, srcdim, DType> &src1,
  ...
  TypeCheckPass<cdim < srcdim && ExpInfo<LhsExp>::kDim == srcdim>
      ::Error_Expression_Does_Not_Meet_Dimension_Req();
  return concatlxgExp<LhsExp, RhsExp, Device, DType, srcdim, srcdim - cdim>
      (src1.self(), src2.self());
}
//------------------------
//  engine plugin
...
template<typename LhsExp, typename RhsExp,
         typename Device, typename DType,
         int srcdim, int dimsrc_m_cat>
struct ShapeCheck<srcdim, concatlxgExp<LhsExp, RhsExp, Device, DType, srcdim, dimsrc_m_cat> >{
  inline static Shape<srcdim> Check(const concatlxgExp<LhsExp, RhsExp,
...
template<typename LhsExp, typename RhsExp,
         typename Device, typename DType,
         int srcdim, int dimsrc_m_cat>
struct StreamInfo<Device, concatlxgExp<LhsExp, RhsExp, Device, DType, srcdim, dimsrc_m_cat> >{
  inline static Stream<Device> *
  Get(const concatlxgExp<LhsExp, RhsExp, Device, DType, srcdim, dimsrc_m_cat> &t) {
...
template<typename LhsExp, typename RhsExp,
         typename Device, typename DType,
         int srcdim, int dimsrc_m_cat>
struct ExpInfo<concatlxgExp<LhsExp, RhsExp, Device, DType, srcdim, dimsrc_m_cat> >{
...
template<typename LhsExp, typename RhsExp,
         typename Device, typename DType,
         int srcdim, int dimsrc_m_cat>
struct Plan<concatlxgExp<LhsExp, RhsExp, Device, DType, srcdim, dimsrc_m_cat>, DType> {
 public:
  static const int dimcat = srcdim - dimsrc_m_cat;
  explicit Plan(const concatlxgExp<LhsExp, RhsExp, Device, DType, srcdim, dimsrc_m_cat> &e)
...
// specialize for concatlxg in x
template<typename LhsExp, typename RhsExp,
         typename Device, typename DType,
         int srcdim>
struct Plan<concatlxgExp<LhsExp, RhsExp, Device, DType, srcdim, 1>, DType> {
 public:
  explicit Plan(const concatlxgExp<LhsExp, RhsExp, Device, DType, srcdim, 1> &e)
      : src1_(MakePlan(e.src1_)), src2_(MakePlan(e.src2_)),
...
#endif  // MSHADOW_EXTENSION_CONCATLXG_H_
