...
#include <functional>
#include <utility>
#include <../src/symbol/static_graph.h>
...
//    printf("num_args length:%d",num_args);
    for (mx_uint i = 0; i < num_args; ++i) {
//      printf("shape_data:%d",i);
//      printf("shape_data:%s",(keys[i]));
...
//by lxg
//2016-3-2
int MXSymbolPrintVector(mx_uint len,mx_uint *nums){
	API_BEGIN();
	std::vector<uint32_t> nums_sym;
//	uint32_t *nums_args = (uint32_t*)nums;
	for(mx_uint i=0;i<len;i++){
		nums_sym.push_back(nums[i]);
	}
	Symbol::PrintVector(nums_sym);
	API_END();
}

//lxg
//2016-3-2
int MXScalaSymbolIsAtomic(SymbolHandle symbol,mx_uint *out){
    API_BEGIN();

    Symbol *s = static_cast<Symbol*>(symbol);

    if(s->isAtomic())
    	*out = static_cast<mx_uint>(1);
    else
    	*out = static_cast<mx_uint>(0);
...
//lxg
//2016-3-30
int MXScalaSymbolIsVariable(SymbolHandle symbol,mx_uint *out){
    API_BEGIN();
...
// by liuxianggen
// 2015-3-6
int MXScalaOpListArguments(OperatorPropertyHandle opHandle,//the function handle
		mx_uint *out_size,const char ***out_str_array) {

  MXAPIThreadLocalEntry *ret = MXAPIThreadLocalStore::Get();// help to hold return information
  API_BEGIN();

  OperatorProperty *op = static_cast<OperatorProperty*>(opHandle);

  ret->ret_vec_str = op->ListArguments();
  ret->ret_vec_charp.clear();
  for (size_t i = 0; i < ret->ret_vec_str.size(); ++i) {
      ret->ret_vec_charp.push_back(ret->ret_vec_str[i].c_str());
  }
  *out_size = static_cast<mx_uint>(ret->ret_vec_charp.size());
  *out_str_array = dmlc::BeginPtr(ret->ret_vec_charp);//BeginPtr returns the pointer
...
// by liuxianggen
// 2016-4-8
int MXScalaOpListAuxiliaryStates(OperatorPropertyHandle opHandle,//the function handle
		mx_uint *out_size,const char ***out_str_array) {
  ...
  ret->ret_vec_str = op->ListAuxiliaryStates();
  ret->ret_vec_charp.clear();
  for (size_t i = 0; i < ret->ret_vec_str.size(); ++i) {
      ret->ret_vec_charp.push_back(ret->ret_vec_str[i].c_str());
  }
...
// by liuxianggen
// 2016-3-8
int MXScalaOpInit(OperatorPropertyHandle OpHandle,
                               mx_uint num_param,
                               const char **keys,
                               const char **vals
                               ) {
...
// by liuxianggen
// 2016-3-8
int MXScalaOpPrintParam(OperatorPropertyHandle OpHandle) {

  API_BEGIN();
  OperatorProperty *op = static_cast<OperatorProperty*>(OpHandle);
  std::map<std::string, std::string> map_param = op->GetParams();
...
//by liuxianggen
//2016-3-9
int MXScalaCreateOperatorProperty(AtomicSymbolCreator creator,OperatorPropertyHandle *out) {
...
int MXScalaSGInferShape(StaticGraphHandle handle,
							mx_uint num_arg_nodes,
							mx_uint num_args,
							const mx_uint *keys_arr,
							const mx_uint *arg_ind_ptr,
							const mx_uint *arg_shape_data,
							mx_uint *in_shape_size,
							const mx_uint **in_shape_ndim,
	...
	StaticGraph *sg = static_cast<StaticGraph*>(handle);
	MXAPIThreadLocalEntry *ret = MXAPIThreadLocalStore::Get();
	bool succ;
	API_BEGIN();
	//  if (keys == nullptr && num_args != 0) {
...
// test operator transport succeed!
//	for (auto nod : sg->nodes){
//		std::cout<<nod.name;
//		if(nod.op!= nullptr){
...
//by liuxianggen
//2016-3-23
int MXScalaOPCopy(OperatorPropertyHandle ophandle,OperatorPropertyHandle *out) {
 ...
 * 2016-3-30
 *
 * StaticGraph *out_graph,
 */
int MXScalaToStaticGraph(StaticGraphHandle *handle,int *arg_node_sg,int arg_node_sg_len,int heads_len,int *heads_source,int *heads_index,
		OperatorPropertyHandle *nods_opHandles,int nods_name_len,const char **nods_name,int *nods_inputs_len_arr,int *nods_inputs_source_ids, int *nods_inputs_indexs,
		int *nods_backward_source_ids, int *nods_attr_len_arr,const char **nods_attrs_keys,const char **nods_attrs_values){
	API_BEGIN();
//	printf(" A ");

	StaticGraph *out_graph = new  StaticGraph();
...
//	arg_nodes.clear();
	...
	//arg_nodes
	for(int i = 0;i<arg_node_sg_len;i++)
		arg_nodes.push_back((uint32_t)arg_node_sg[i]);
//	printf(" B ");
...
//	out_graph->heads.clear();
	...
	for (int i=0;i<heads_len;i++) {
	    ...
	    e.source_id = heads_source[i];
	    e.index = heads_index[i];
...
//	printf(" C ");
	...
	int count_attr = 0;
	int count_inputs = 0;
	out_graph->nodes.resize(nods_name_len);
	for(int i=0;i<nods_name_len;i++){
		out_graph->nodes[i].name = nods_name[i];
		if(nods_opHandles[i]!=nods_opHandles[nods_name_len]){
			OperatorProperty *op = static_cast<OperatorProperty*>(nods_opHandles[i]);
			out_graph->nodes[i].op.reset(op->Copy());
		}else {
		    out_graph->nodes[i].op.reset(nullptr);
		    }

		//input
		auto &inputs = out_graph->nodes[i].inputs;
		inputs.clear();

		for (int j = 0;j<nods_inputs_len_arr[i];j++) {
		      StaticGraph::DataEntry e;
		      e.index = nods_inputs_indexs[count_inputs];
		      e.source_id = nods_inputs_source_ids[count_inputs++];
		      inputs.push_back(e);
//		      printf("input index:%d\n",e.index);
//		      printf("input source_id:%d\n",e.source_id);
		    }
		out_graph->nodes[i].backward_source_id = nods_backward_source_ids[i];
//		printf("backward_source_id:%d",out_graph->nodes[i].backward_source_id);
//		printf(" D ");

		std::map<std::string, std::string> map_attr;
		for (int j = 0;j<nods_attr_len_arr[i];j++) {
			out_graph->nodes[i].attr[nods_attrs_keys[count_attr]] = *(nods_attrs_values[count_attr]);  //  NOLINT(*)
		}
//		out_graph->nodes[i].attr = map_attr;
	}
...
int MXScalaExecutorBind(StaticGraphHandle sg_handle,
                   int dev_type,
                   int dev_id,
                   mx_uint len,
                   NDArrayHandle *in_args,
                   ...
                   NDArrayHandle *aux_states,
                   ExecutorHandle *out) {
  return MXExecutorBindX(sg_handle,
                         dev_type, dev_id,
                         0, nullptr, nullptr, nullptr,
                         len, in_args, arg_grad_store, grad_req_type,
                         aux_states_len, aux_states, out);
...
int MXScalaExecutorBindX(StaticGraphHandle sg_handle,
                    int dev_type,
                    int dev_id,
                    mx_uint num_map_keys,
                    const char** map_keys,
                    ...
                    ExecutorHandle *out) {
  API_BEGIN();
//  StaticGraph *staticGraph = static_cast<StaticGraph*>(sg_handle);
  Context ctx = Context::Create(static_cast<Context::DeviceType>(dev_type), dev_id);
  std::map<std::string, Context> ctx_map;
  for (mx_uint i = 0; i < num_map_keys; ++i) {
    ctx_map[std::string(map_keys[i])] = Context::Create(
...
//  printf(" A ");
//  if(1) return 0;//passed
  *out = Executor::Bind(sg_handle, ctx, ctx_map, in_args_vec,
                        arg_grad_vec, grad_req_vec, aux_states_vec);
  API_END();
}
/**
 ...
 * 2016-4-5
 */
int MXNDArraySetData(NDArrayHandle handle,int index,float source) {
	 API_BEGIN();
	 NDArray *arr = static_cast<NDArray*>(handle);
	 CHECK(arr->ctx().dev_mask() == cpu::kDevMask)
			<< "MXNDArrayGetData can only be called for NDArray on CPU";
	 const TBlob &b = arr->data();
	 CHECK(b.CheckContiguous());
	 float *out_pdata = b.FlatTo2D<cpu, mx_float>().dptr_;
	 out_pdata[index] = source;
	 API_END();
}
