...
object Model {
  ...
  private[brainmatrix] def serialize(symbol: Symbol,
  ...
  private[brainmatrix] def deserialize(bytes: Array[Byte]):
    (Symbol, Map[String, NDArray], Map[String, NDArray]) = {
   ...
   */
  private[brainmatrix] def createKVStore(kvStore: String,
                                   numDevice: Int,
   ...
   * @return Option of created [[KVStore]] and whether or not update weight on it
   */
  private[brainmatrix] def createKVStore(kvStore: KVStore): (Option[KVStore], Boolean) = {
  ...
  private[brainmatrix] def trainMultiDevice(symbol: Symbol, ctx: Array[Context],
    ...
    for (epoch <- beginEpoch until endEpoch) {
      ...
      evalData.foreach { evalDataIter =>
        ...
        val (name, value) = evalMetric.get
        println(s"Epoch[$epoch] Validation-$name=$value")
      }
