...
object GanForMnist {
  ...
  def makeDcganSym(oShape: Shape, ngf: Int = 128, finalAct: String = "sigmoid",
      eps: Float = 1e-5f + 1e-12f): (Symbol, Symbol, Symbol) = {
    ...
    val group = Symbol.Group(net,gout)
...
//            logger.info(s"epoch: $epoch, iter $t, metric=$value")
            ...
            CVTool.saveFlattenImage(gMod.midOutput,"output/gdeconv14.jpg")
