...
/**
 * NDArray API of brainmatrix
 * @author Yizhi Liu, Yuan Tang
 */
object NDArray {
  private val logger = LoggerFactory.getLogger(classOf[NDArray])

  private[brainmatrix] val DTYPE_NATIVE_TO_MX: Map[Class[_ >: Float with Int with Double], Int] = Map(
    classOf[Float] -> 0,
    classOf[Double] -> 1,
    classOf[Int] -> 4)

  private[brainmatrix] val DTYPE_MX_TO_NATIVE: Map[Int, Class[_ >: Float with Int with Double]] = Map(
    0 -> classOf[Float],
    1 -> classOf[Double],
    2 -> classOf[Float],
    3 -> classOf[Int],
  ...
  def invokeGenericFunc(funcName: String,
    ...
    function match {
      ...
      case _ => {
    	  println(function)
    	  throw new IllegalArgumentException(s"call $funcName as generic function")
  ...
  private def makeNdarrayFunction(handle: FunctionHandle): (String, NDArrayFunction) = {
    ...
    //     println("NDArray function defination:\n{}", docStr)
   ...
   * @author liuxianggen
   * @date 20160729
   * @brief create a ndarray that is a diagonale matrix
   * @example
   *   [1,0
   *    0 ,1]
   * @note
   */
  def diag(shape: Shape, ctx: Context = null): NDArray = {
    require(shape.length == 2, "required:length=2")
    val (rows, cols) = (shape(0), shape(1))
    val len = math.min(rows, cols)
    val arr = zeros(Shape(rows, cols), ctx)
    for (i <- 0 until len) {
      arr(i, i) = 1
    }
    arr
   ...
   *
   */
  def range(from1: Int, till1: Int): NDArray = {

    val arr = empty(Shape(till1 - from1))
    var idx = 0
    for (i <- from1 until till1) {
      arr(idx) = i
      idx += 1
    }
   ...
   * by liuxianggen
   * generate the NDArray
   * two dimensional(dup,xxx) Shape from 0 to rtill
   */
  def range(from1: Int, till1: Int, dup: Int): NDArray = {
    val arr = empty(Shape(dup, (till1 - from1)))
    for (j <- 0 until dup) {
      var idxj = 0
      for (i <- from1 until till1) {
        arr(j, idxj) = i
   ...
   * by liuxianggen
   * generate the NDArray
   * one dimensional Shape from 0 to rtill
   */
  def range(rtill: Int): NDArray = {
    val from1 = 0
    val arr = empty(Shape(rtill - from1))
    var idx = 0
    for (i <- from1 until rtill) {
      arr(idx) = i
      idx += 1
    }
  ...
  def rangeRows(from1: Int, till1: Int, cols: Int): NDArray = {
    val rows = till1 - from1
    val data = NDArray.ones(Shape(rows, cols))

    for (i <- 0 until rows) {
      for (j <- 0 until cols) {

        data(i, j) = from1 + i + (scala.util.Random.nextFloat - 0.5f)
        //        println(data(i, j))
      }
    }
    data
  ...
  /**
   * author:guoshen
   * Take sigmod of the src
   * @param src Source ndarray
   * @return a new [[NDArray]]
   */
  def sigmod(src: NDArray): NDArray = {
    var res = NDArray.ones(src.shape,src.context) / (NDArray.exp(-src) + 1)
    res
  }
  /**
   * author:guoshen
   ...
   * @return a new [[NDArray]]
   */
  def tan(src: NDArray): NDArray = {
    NDArray.invokeUnaryFunc("sin", src) / NDArray.invokeUnaryFunc("cos", src)
  }
   ...
   * @return a new [[NDArray]]
   */
  def tanh(src: NDArray): NDArray = {
    var temp = NDArray.invokeUnaryFunc("exp", src)
    var temps = temp.*(temp)
    (temps.-(NDArray.ones(temps.shape))) / (temps.+((NDArray.ones(temps.shape))))
  }
  /**
   * author:guoshen
   * Take a transpose of the src
   * @param src Source ndarray
   * @return a new [[NDArray]]
   */
  def transpose(src: NDArray): NDArray = {

	  NDArray.invokeUnaryFunc("transpose", src)

  }

  /**
   * author:guoshen
   * Return a NDArray copy of the src
   * @param src Source ndarray
   * @return a new [[NDArray]]
   */
  def copy(src: NDArray): NDArray = {
    var cpy = NDArray.array(src.toArray, src.shape)
    cpy
  }
  
  /**
   * author:liuxianggen
   * a new operation of my own
   */
  
  def integate_lxg(array: NDArray,array1: NDArray): NDArray = {
    NDArray.invokeGenericFunc("integate_lxg", Array(array,array1,3f))(0)
  }
   ...
   * the src and the dst dimension should be 2.  
   * set the slice things which will offer convinience to integrate
   */
  
  def setColumnSlice(dst: NDArray,src: NDArray,idx:Int): NDArray = {
	  if(dst.shape(0)!=src.shape(0)){
	 	  throw new  java.lang.Exception("the length of row should be the same!")
	  }
	   
	  if(idx<0 || (idx+src.shape(1)) > dst.shape(1)){
	 	  throw new  java.lang.Exception("the idx must be integer and idx+src.shape(1) less than dst.shape(1)")
	  }
	   
    NDArray.invokeGenericFunc("setslice_lxg", Array(dst,src,idx.toFloat))(0)
  }
  
  
  def  Normalize(src: NDArray):NDArray = {
	  val rows = src.shape(0)
	  for(i<- 0 until rows){
	 	src.slice(i) /= NDArray.sum(src.slice(i)).toScalar	  
	  }
	  src
  }
   ...
   * @return a new [[NDArray]]
   * 
   * bref: return the max element index of each line 
   */
...
/**
 * NDArray object in brainmatrix.
 * NDArray is basic ndarray/Tensor like data structure in brainmatrix. <br />
 * <b>
...
class NDArray private[brainmatrix] (private[brainmatrix] val handle: NDArrayHandle,
                                    val writable: Boolean = true) {
  // record arrays who construct this array instance
  // we use weak reference to prevent gc blocking
  private[brainmatrix] val dependencies = mutable.HashMap.empty[Long, WeakReference[NDArray]]
   ...
   * by liuxianggen
   * 2016-4-4
   * due to bugging mxNDArrayGetData, use mxNDArraySyncCopyToCPU instead of 
   * 
   */
  /// apply methods ///
  def apply(i1: Int): Float = {
    assert(i1 < size, "index out of bound")
    val data = Array.ofDim[Float](size)
    checkCall(_LIB.mxNDArraySyncCopyToCPU(handle, data, size))
    data(i1)
  }

  def apply(i1: Int, i2: Int): Float = apply(foldDim(i1, i2))
  def apply(i1: Int, i2: Int, i3: Int): Float = apply(foldDim(i1, i2, i3))
  def apply(i1: Int, i2: Int, i3: Int, i4: Int): Float = apply(foldDim(i1, i2, i3, i4))

  /// update methods ///
  def update(i1: Int, x: Float): Unit = {
    assert(i1 < size, "index out of bound")
    _LIB.mxNDArraySetData(this.handle, x, i1)
  }
  def update(i1: Int, i2: Int, x: Float): Unit = update(foldDim(i1, i2), x)
  def update(i1: Int, i2: Int, i3: Int, x: Float): Unit = update(foldDim(i1, i2, i3), x)
  def update(i1: Int, i2: Int, i3: Int, i4: Int, x: Float): Unit = update(foldDim(i1, i2, i3, i4), x)

  def foldDim(arr: Int*): Int = {
    arr.length match {
      case 1 => arr(0)
      case 2 => arr(0) * shape(1) + arr(1)
      case 3 => arr(0) * shape(1) * shape(2) + arr(1) * shape(2) + arr(2)
      case 4 => arr(0) * shape(1) * shape(2) * shape(3) + arr(1) * shape(2) * shape(3) + arr(2) * shape(3) + arr(3)
      case _ => -1 // Undefined
    }
  }
   ...
   */
  def context: Context = {
    val devTypeId = new Base.RefInt
    val devId = new Base.RefInt
...
//    require(shape == Shape(1), "The current array is not a scalar")
	  require(size == 1, "The current array is not a scalar")
    this.toArray(0)
  }
...
//    	  println(s"this:${this.shape}")
//    	  println(s"other:${other.shape}")
  ...
  override def toString: String = {
    ...
    val res = this.shape.toVector match {
      case Vector(_) => {
        var s = "the value is: \n"
        this.toArray.foreach { x => s += " " + x }
        s
      }
      case Vector(x, y) => {
        var s = "the value is(two dimension): \n"
        var count = 0
        for (j <- 0 until x) {
          for (i <- 0 until y) {
            s += this.toArray(count) + "  "
            count += 1
          }
          s += "\n"
        }
      ...
      case Vector(x, y, z, _*) => {
        var s = ">= 3 dimension,the first 3 dimension is: \n"
        var count = 0
        for (k <- 0 until x) {
          s += "[\n"
          for (j <- 0 until y) {
      ...
      case _ => " this NDArray can not be converted to string"
    }
...
case class ExtendNDArrayFunction(handle: FunctionHandle,
//                                  acceptEmptyMutate: Boolean,
                                  nMutateVars: Int,
                                  useVarsRange: Range,
                                  scalarRange: Range) extends NDArrayFunction
