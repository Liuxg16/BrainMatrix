package thu.brainmatrix.neuralStyle
...
object NeuralStyle {
  ...
  def preprocessContentImage(path: String, longEdge: Int, ctx: Context): NDArray = {
    ...
    sample.set(datas)
    println(s"resize the content image to ${(newHeight, newWidth)}")
    sample
  }
  ...
  def saveImage(img: NDArray, filename: String, radius: Int): Unit = {
    println(s"save output to $filename")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser: CmdLineParser = new CmdLineParser(alle)
      ...
      println(s"start training arguments $alle")
      ...
      while (e < alle.maxNumEpochs && !trainingDone) {
        ...
        oldImg.set(img)
        println(s"epoch $e, relative change $eps")
...
class NeuralStyle {
  ...
  private val contentImage: String = "./data/ating1.jpg"
  @Option(name = "--style-image", usage = "the style image")
  private val styleImage: String = "./data/starry_night.jpg"
  ...
  private val maxNumEpochs: Int = 2
