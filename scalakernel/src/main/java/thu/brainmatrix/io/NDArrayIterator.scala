 ...
 */
class NDArrayIterator (data: IndexedSeq[NDArray],dataName:String = "data", label: IndexedSeq[NDArray] = null,
                  labelName:String = "label",private val dataBatchSize: Int = 1, shuffle: Boolean = false,
                  lastBatchHandle: String = "pad") extends DataIter {
  private val logger = LoggerFactory.getLogger(classOf[NDArrayIterator])
  ...
  private def getShape(dataItem: (String, NDArray)): (String, Shape) = {
    ...
    (dataItem._1, Shape(dataBatchSize) ++ newShape)
  }
